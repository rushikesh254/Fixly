...
import { getMyServices, updateService } from "../../api/services";
...
function ProviderDash() {
  ...
  const fetchDashboard = useCallback(
    () =>
      Promise.all([getMyBookings(), getMyProviderProfile(), getMyServices()]).then(
        ([bookingsRes, profileRes, servicesRes]) => ({
          bookings: bookingsRes.data.bookings.map(normalizeBooking),
          provider: profileRes.data.provider,
  ...
  const { data, loading, error, refetch, setData } = useFetch(fetchDashboard);

  // memoised so the derived lists below keep a stable dependency
  const myBookings = useMemo(() => data?.bookings || [], [data]);
  const provider = data?.provider || null;
  const services = data?.services || [];

  // the toggle reflects the services, a provider is available when any of them is
  const availableToday = services.some((service) => service.availableToday);
  const [isTogglingAvailability, setIsTogglingAvailability] = useState(false);

  const toggleAvailability = async () => {
    if (services.length === 0) {
      toast.error("Add a service before setting your availability.");
      return;
    }
    ...
    try {
      // availability lives on each service, so they are all kept in step
      await Promise.all(
        services.map((service) => {
          const formData = new FormData();
          formData.append("availableToday", String(next));
          return updateService(service._id, formData);
        }),
      );
      setData((prev) => ({
        ...prev,
      ...
      toast.success(
        next
          ...
          : "You are no longer marked available today.",
      );
    } catch (err) {
      toast.error(getApiErrorMessage(err, "Could not update your availability."));
    } finally {
  ...
  return (
    <div className="min-h-screen bg-gray-50 px-5 py-8 sm:px-8 lg:px-15">
      ...
      <div className="rounded-2xl bg-blue-600 px-6 py-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between sm:px-10">
        ...
        <div className="flex items-center gap-3 w-fit rounded-full bg-white/10 px-4 py-2.5">
          ...
          <button
            ...
            disabled={isTogglingAvailability || loading}
            className={`relative h-6 w-11 shrink-0 cursor-pointer rounded-full transition disabled:cursor-not-allowed disabled:opacity-60 ${
      ...
      {/* a provider awaiting approval is not discoverable yet, so say so */}
      {provider && provider.providerStatus !== "approved" && (
        <div className="mt-6 rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4">
          ...
          <p className="mt-0.5 text-[13px] text-amber-600">
            {provider.providerStatus === "rejected"
              ? "Customers cannot see your services. Contact support if you think this is a mistake."
              : "You can set up your profile and services now. Customers will be able to find you once an admin approves your account."}
          </p>
        </div>
      )}

      {loading && <Loader label="Loading your dashboard..." className="mt-8" />}
